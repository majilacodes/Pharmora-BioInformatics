...
if targetname:
    ...
    if target_index < len(targets):
        ...
        trained_model, X = model(df_2class)
        ...
        st.header("Predict pIC50 for New Molecules")
        user_smiles = st.text_area("Enter SMILES of molecules (one per line):")
        
        if user_smiles:
            user_mols = user_smiles.split('\n')
            user_df = pd.DataFrame({'canonical_smiles': user_mols})
            
            # Calculate Lipinski descriptors for user input
            user_lipinski = lipinski(user_df['canonical_smiles'])
            user_df = pd.concat([user_df, user_lipinski], axis=1)
            
            # Ensure user input has the same features as the trained model
            user_features = user_df[X.columns]
            
            # Make predictions
            predictions = trained_model.predict(user_features)
            
            user_df['Predicted pIC50'] = predictions
            st.write(user_df)
